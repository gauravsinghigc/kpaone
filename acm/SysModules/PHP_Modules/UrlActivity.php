...
function ClearFilters($InitiateFilter = null, $Redirect = "index.php")
{
    if ($InitiateFilter != null) {
        ...
        if (isset($_GET["$InitiateFilter"])) {
            ...
            if (!empty($_GET)) {
                $filterDisplay .= '<div class="row bg-warning filter-block">';
                $filterDisplay .= '<div class="col-md-10">';
                $filterDisplay .= '<p class="mb-0 p-1 text-white">';
                ...
                $filterDisplay .= '<a href="' . $Redirect . '" class="btn fs-13 btn-xs btn-light pull-right">';
